...
public class YxlController {
	...
	public @ResponseBody PageList<YxlDocSearch> getDocList(Model model,@RequestParam(required=false,defaultValue="-1") int page ,
    ...
    public @ResponseBody PageList<DocListVO> queryDocList(Model model,@RequestParam(required=false,defaultValue="-1") int page ,
...
//    	        }
    ...
    public @ResponseBody YxlDoc docText(Model model, Long docId){
        YxlDoc doc=yxlDocService.queryById(docId);
        if(doc==null){
            return new YxlDoc();
        }
