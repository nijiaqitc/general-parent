			...
			charToOther:function(type){
				...
				if(startNode.nodeType==3&&startNode==endNode){
					...
					if(range.startOffset==0&&range.endContainer.length==range.endOffset){
						...
						if(util.checkIsEmpty(t1.previousSibling)){
							util.removeNode(t1.previousSibling);
						}
					...
					if(startNode.nodeType==3&&range.startOffset!=0){
						...
						if(util.checkIsEmpty(textNode.nextSibling)){
							util.removeNode(textNode.nextSibling);
						}
						startNode=textNode;
					}	
			...
			deleteRow:function(){
				...
				for(var i=rangeTable.beginRowIndex;i<=rangeTable.endRowIndex;i++){
					...
					for(var k in addRow){
						...
						rangeTable.tempTable[nextTrNum][k.split(",")[1]]={col:addTd.colSpan,row:addTd.rowSpan,td:addTd};
